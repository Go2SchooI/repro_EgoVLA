#!/bin/bash
set -euo pipefail

n_node=1
n_gpu=${N_GPU:-4}
bs=${PER_DEVICE_BS:-1}
grad_accum=${GRAD_ACCUM_STEPS:-4}
num_epochs=${NUM_EPOCHS:-10}

CHECKPOINTS_ROOT=${CHECKPOINTS_ROOT:-./checkpoints}
RUN_NAME=${RUN_NAME:-otv-fixed-set-subset-4gpu-wandb}
OUTPUT_DIR=$CHECKPOINTS_ROOT/$RUN_NAME
MODEL_PATH=${MODEL_PATH:-$CHECKPOINTS_ROOT/mix4data-30hz-transv2update2-fingertip-20e-hdof5-3d200-rot5-lr1e-4-h5p30f1skip6-b16-4}
DATA_ROOT=${DATA_ROOT:-./data/EgoVLA_SIM_Processed}
WANDB_PROJECT_NAME=${WANDB_PROJECT_NAME:-ego_manip_release}

echo "number of nodes: $n_node"
echo "gpus per node: $n_gpu"
echo "per device batch size: $bs"
echo "gradient accumulation steps: $grad_accum"
echo "num train epochs: $num_epochs"
echo "output dir: $OUTPUT_DIR"

if [ ! -d "$MODEL_PATH" ]; then
    echo "Missing pretrained checkpoint directory: $MODEL_PATH"
    exit 1
fi

if [ ! -d "$DATA_ROOT/HF_images" ]; then
    echo "Missing processed image dataset: $DATA_ROOT/HF_images"
    exit 1
fi

if [ ! -d "$DATA_ROOT/HF_hand_FIXED_SET_MIX_train" ]; then
    echo "Missing processed hand dataset: $DATA_ROOT/HF_hand_FIXED_SET_MIX_train"
    exit 1
fi

if [ ! -f "$DATA_ROOT/hf_images_mapping.pkl" ]; then
    echo "Missing image mapping file: $DATA_ROOT/hf_images_mapping.pkl"
    exit 1
fi

if [ -z "${WANDB_API_KEY:-}" ] && [ -f .wandb_api ]; then
    export WANDB_API_KEY=$(cat .wandb_api)
fi

if [ -z "${WANDB_API_KEY:-}" ]; then
    echo "Missing WANDB_API_KEY. Export it or create .wandb_api in the repo root."
    exit 1
fi

export WANDB_PROJECT=$WANDB_PROJECT_NAME
export WANDB_MODE=${WANDB_MODE:-online}

torchrun --nnodes=$n_node --nproc_per_node=$n_gpu --master_port=25001 \
    human_plan/vila_train/train_mem.py \
    --deepspeed VILA/scripts/zero3.json \
    --model_name_or_path $MODEL_PATH \
    --version qwen2 \
    --vision_tower google/siglip-so400m-patch14-384 \
    --data_mixture otv_sim_fixed_set_FIXED_SET_MIX_train \
    --eval_data_mixture otv_sim_fixed_set_FIXED_SET_MIX_train_sub50 \
    --mm_vision_select_feature cls_patch \
    --mm_projector mlp_downsample \
    --tune_vision_tower True \
    --tune_mm_projector True \
    --tune_language_model True \
    --mm_vision_select_layer -2 \
    --mm_use_im_start_end False \
    --mm_use_im_patch_token False \
    --image_aspect_ratio resize \
    --bf16 True \
    --group_by_modality_length False \
    --output_dir $OUTPUT_DIR \
    --num_train_epochs $num_epochs \
    --per_device_train_batch_size $bs \
    --per_device_eval_batch_size 1 \
    --eval_accumulation_steps 1 \
    --gradient_accumulation_steps $grad_accum \
    --evaluation_strategy steps \
    --eval_steps 500 \
    --save_strategy steps \
    --save_steps 500 \
    --save_total_limit 3 \
    --learning_rate 2e-5 \
    --weight_decay 0.0 \
    --warmup_ratio 0.03 \
    --lr_scheduler_type constant \
    --logging_steps 10 \
    --model_max_length 4096 \
    --gradient_checkpointing True \
    --dataloader_num_workers 4 \
    --lazy_preprocess True \
    --report_to wandb \
    --run_name $RUN_NAME \
    --future_index 1 \
    --predict_future_step 30 \
    --max_action 1 \
    --min_action 0 \
    --add_his_obs_step 5 \
    --add_his_imgs True \
    --add_his_img_skip 6 \
    --num_action_bins 256 \
    --action_tokenizer uniform \
    --invalid_token_weight 0.1 \
    --mask_input True \
    --add_current_language_description False \
    --traj_decoder_type transformer_split_action_v2 \
    --raw_action_label True \
    --traj_action_output_dim 48 \
    --input_placeholder_diff_index True \
    --ee_loss_coeff 20.0 \
    --hand_loss_coeff 5.0 \
    --hand_loss_dim 6 \
    --ee_2d_loss_coeff 0.0 \
    --ee_rot_loss_coeff 5.0 \
    --hand_kp_loss_coeff 0.0 \
    --next_token_loss_coeff 0.0 \
    --traj_action_output_ee_2d_dim 0 \
    --traj_action_output_ee_dim 6 \
    --traj_action_output_hand_dim 30 \
    --traj_action_output_ee_rot_dim 12 \
    --ee_rot_representation rot6d \
    --correct_transformation True \
    --include_2d_label True \
    --include_rot_label True \
    --use_short_language_label True \
    --no_norm_ee_label True \
    --tf32 True \
    --merge_hand True \
    --use_mano True \
    --sep_proprio True \
    --sep_query_token True \
    --loss_use_l1 True \
    --ee_movement_mask_idx 29
